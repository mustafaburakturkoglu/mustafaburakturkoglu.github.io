(function($) {
  ...
  // Initialize slick slider dbio-latest-news-block LatestNews
  const initializeLatestNewsCarousel = ($block) => {
	   $('.dbio_latest_news_block_container .twl_carousel').each(function(key, item) {    
 		 // $('#'+item.id).slick({
			// this.parent().find()
		 $('#'+item.id).not('.slick-initialized').slick({	
				slidesToShow: 3,  
				slidesToScroll: 1,
				appendArrows: $(this).parent().find('.dev_latest_news_nav'),  
  ...
  $(document).ready(function() {
      initializeLatestNewsCarousel();
  });

  // Initialize dynamic TestimonialCarousel preview (on Gutenberg editor).  
  if (window.acf) { 
    window.acf.addAction('render_block_preview/type=dbio-latest-news-block', initializeLatestNewsCarousel);
  }
